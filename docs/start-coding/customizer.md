...
HCL Connections Customizer is a middleware proxy service that enables the customization of the HCL Connections user experience (UX). 
In essence Customizer acts as a proxy between HCL Connections and the end-user, which gives it the ability to intercept and modify 
...
The HCL Connections Customizer model is simple – the service performs customizations by injecting JavaScript, CSS or 
other web resources into the HTML pages returned by HCL Connections in response to end-user requests, i.e. the requests 
...
applications stored inside the HCL Connections Application Registry - aka App Reg.  
...
different HCL Connections services – where Customizer is just one such service. In the cloud, App Reg is available to 
...
For Customizer applications, the path property value is used to identify a path element in the HCL Connections request URL, which in most use cases corresponds to a standard HCL Connections component. 

Consider the URLs displayed in Listing 3 - these sample URLs follow a clear pattern where the next element after the HCL Connections cloud domain name identifies the Connections component or application handling the request. The possible values of this element map to the path values enumerated in Listing 2, i.e. homepage, communities, files, etc.
...
It is important to realise that user names are not unique within an organization so it’s possible to inadvertently target unintended users by employing this technique, i.e. any users of the same name will see the extension. To avoid this it is possible to apply a precise filter by using the user-id match property instead. Note that the term “user id” is sometimes referred to as “subscriber id” in the HCL Connection UI and documentation. 
...
Figure 1 – HCL Customizer Request Life Cycle

![Figure 1 – HCL Customizer Request Life Cycle](./CustomizerFigure1.png)
...
For HCL Connections Cloud, any files declared in the include-files property list are stored in one of two locations:

a)	a private HCL GitHub organization (i.e. github.ibm.com - accessible only to IBM)

b)	a public HCL Connections GitHub organization - https://github.com/hclcnx

The include-repo payload property value identifies the name of the actual repository. For example, in Listing 1 and Listing 4 you see an include-repo object with a name value of "global-samples" being used. This is a reference to a repository on github.ibm.com that contains ready-made samples that any HCL Connections Cloud tenant can use in a Customizer app. "Hello World", "FlipCard" and the other samples featured later in the Standard Samples section are all located in this repository.  HCL Customizer resolves the GitHub organization referred to in the JSON markup –i.e. whether it is in the private or public location. HCL has control over the repositories that care created in both locations  so no duplicate names are allowed.

2.	How do they get there?

Customizer assets like the aforementioned global-samples are directly provisioned to github.ibm.com by the HCL Customizer team. Since this is a private GitHub organization you cannot explore it to discover what repositories are available, but you become aware of them through public samples, documentation and other enablement materials (such as this). It is envisaged that an enhanced App Reg IDE may expose these repositories through the UI in a future release.  

On the other hand you can freely explore the assets available on the public HCL Connections Developers GitHub organization (see Figure 2). By default you are free to leverage any Customizer repository within this organization or to collaborate with the HCL Customizer team to create your own repo in this location. This could be a fork of an existing repository or a brand new repo created for you from scratch, depending on your needs.
...
In order to inject your own include-files into a Customizer app you need a GitHub repository on the public github.com/hclcnx organization. Typically developers have their own repo that they share with HCL – the step by step procedure is as follows:

a.	Share your repo with HCL – add "ibmcndev" as a collaborator

b.	HCL (hclcnx) then creates a fork of your repository under github.com/hclcnx and grants you read access by default.

c.	You can continue to work on your extension using your original repo for your source code activity, but once you are ready to deliver to HCL Cloud you must issue a pull request to IBM. 

d.	HCL merges your pull request once acceptance criteria are met.

e.	Upon merge, the repo files are automatically pushed to HCL Customizer via a webhook. 

f.	Rinse & repeat starting at Step (c) for extension updates.

Step (c) requires you to issue a Pull Request across forks (in GitHub parlance). The key thing to remember is that your original repo which contains the latest changes is always the “head fork”, while the “base fork” must refer to the repo on github.com/hclcnx.

Step (d) involves an initial lightweight summary review by HCL which looks at various aspects of the proposed customization, primarily from a performance, security and documentation standpoint. However ultimate responsibility for the quality and behaviour of the app remains that of the customer who creates or adopts the customization. The review process by HCL provides no guarantee whatsoever of protection against adverse security or performance impacts.

Figure 2 – HCL Connections Developers Organization on GitHub

![Figure 2 – HCL Connections Developers Organization on GitHub](./CustomizerFigure2.png)

TIP: More information on how to integrate your Customizer include files with HCL Connections Cloud is available in video for on opencode4connections.org:

https://opencode4connections.org/oc4c/customizer.xsp?key=ccc-episode2

### Restricting Access to Include-Files

By default the contents of any repository in either GitHub organization are available for use by Customizer apps by any HCL Connections Cloud tenant. This is a very flexible and convenient model but may not always be the desired solution for every situation. Some tenants may prefer to keep the include-files for Customizer apps private to themselves, or restrict usage to a subset of tenants. Different solutions exist to address these needs:

1.	Access Control Lists for Tenant Organizations

Access Control Lists (ACLs) are used to manage access to a particular object. HCL Connections Customizer provides a very simple implementation of an ACL which can control which tenant organizations are allowed to load include files from your repos. All you need to do is to provide an acl.ids file at the root of your project and populate it with the HCL Connections Cloud ids of the tenant organizations to whom you wish to grant access.  
...
GitHub users on a paid GitHub plan have the option of creating private repositories. Private repositories can still be shared with the HCL Connections Developers organization. The private repository will appear in the list of projects under github.com/hclcnx but only administrators of hclcnx will be able to see the contents – i.e. the repo files have no visibility to regular users or to the general public. Even though read access of the source files is restricted via the repository, you will also need to add an acl.ids file should you also wish to prevent runtime access from other tenant organizations.
...
•	HCL Connections classic UI uses Dojo so code is injected into a Dojo structured page
...
![Figure 3 Hello World Extension for HCL Connections Homepage](./CustomizerFigure3.png)

This waitFor()function is thus called passing in the callback function to manipulate the DOM and modify the UI. The interesting part of the callback function (Line 31 as already highlighted) locates a DOM element and assigns “Hello World” as the text content. When this extension is loaded and run by Customizer then the HCL Connections Homepage is modified in the manner shown in Figure 3.

The code injection can be seen by viewing the source of the HCL Connections Homepage in the browser and scrolling to the bottom of the file. The following tag fragment should be evident:
...
NOTE: The alphabetical order of extensions applies across all applications. For example, you may have two separate apps that target the HCL Connections homepage. The extensions defined within both applications will be sorted as a single alphabetical list by the App Registry and returned to Customizer and then injected in that order into the HCL Connections homepage.
...
The sample customizations discussed in this document are available to any HCL Connections Cloud tenant organization. Applying a sample customization is an easy way to get started with HCL Connections Customizer and help you get familiar with the process. Any sample can be used put through its paces by importing the relevant JSON file into an organization’s Application Registry. So for example, you could take a copy of the helloWorld.json file from the helloWorld samples project published on the Customizer GitHub repo and import it into App Reg as follows:

1.	Go to https://github.com/hclcnx/customizer

2.	Navigate to the  helloWorld.json file and copy/paste the contents to a local file

3.	As Admin user in your HCL Connections Cloud organization go to:
...
9.	Refresh the HCL Connections Homepage and verify that the Hello World extension appears

† Note that Step 4 is temporary while the new HCL Connections Pink UI components are being integrated into the cloud.
...
HCL Connections apps but does not include the Administration URLs (aka BSS), or related HCL ICS components like HCL Docs, Meetings, Chat or Verse.  The list may be expanded to include these and possibly other components in the future.
